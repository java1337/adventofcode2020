 ...
 *
 */
class Day20201205(input: List<String>) {

    class BoardingPass(rawInput: String) {
        ...
        init {
            val binaryString = rawInput
                .replace("F", "0")
                .replace("B", "1")
                .replace("L", "0")
                .replace("R", "1")

            seatId = Integer.parseInt(binaryString, 2)
            row = seatId / 8
            columnn = seatId % 8
        }
    }

    private val passes : List<Int>

    val maxBoardingPass : Int
    val missingPass : Int

    init {
        passes = input.map { BoardingPass(it).seatId }.sorted()
        maxBoardingPass = passes.last()

        var previous = passes.first()
        var missingPass = 0
        for (pass in passes) {
            if (pass > previous + 1) {
                missingPass = pass -1
